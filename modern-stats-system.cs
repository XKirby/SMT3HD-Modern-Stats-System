...
namespace ModernStatsSystem
{
    public class ModernStatsSystem : MelonMod
    {
        ...
        private class PatchSetAllPacket
        {
            private static bool Prefix(ref nbActionProcessData_t a, int koukatype, int nskill, int sformindex, int dformindex, int cnt, int frame, float koukaritu)
            {
                if (cnt > 1 && koukatype == 0)
                {
                    datUnitWork_t s = nbMainProcess.nbGetUnitWorkFromFormindex(sformindex);
                    datUnitWork_t d = nbMainProcess.nbGetUnitWorkFromFormindex(dformindex);
                    float powerrate = nbCalc.nbGetAisyoRitu(nskill, sformindex, dformindex);
                    uint baddamage = nbCalc.nbGetKoukaBadDamage(nskill, sformindex, dformindex, powerrate * koukaritu, 0);
                    uint badrecover = nbCalc.nbGetKoukaBadKaifuku(nskill, sformindex, dformindex, powerrate * koukaritu, 0);
                    int hittype = nbCalc.nbGetHitType(a, nskill, sformindex, dformindex);
                    int endframe = frame;
                    if (hittype == 1)
                        { powerrate = nbCalc.nbGetCriticalPow(sformindex, powerrate); }
                    if (dformindex < 0x10)
                    {
                        if (a.data.form.Length <= dformindex)
                            { return false; }
                        if (a.data.party.Length <= a.data.form[dformindex].partyindex)
                            { return false; }
                    }
                    if (a.timelist.Length <= dformindex)
                        { return false; }
                    if (frame < a.timelist[dformindex].lockframe)
                        { frame = a.timelist[dformindex].lockframe; }
                    int i = 0;
                    int chkifdead = 0;
                    int canttarget = 0;
                    int hp = 0, hp2 = 0;
                    int mp = 0, mp2 = 0;
                    int drainedhp = 0, drainedmp = 0;
                    uint badstat = 0;
                    int uniqueid = 0;
                    bool skilltargetchk = false;
                    nbFormation_t form = a.form;
                    do
                    {
                        hp = nbCalc.nbGetKoukaHp(1, nskill, sformindex, dformindex, powerrate, cnt);
                        hp2 = nbCalc.nbGetKoukaHp(0, nskill, sformindex, dformindex, powerrate, cnt);
                        if (baddamage == 0x800)
                        {
                            canttarget = 1;
                            chkifdead = nbCalc.nbGetDeadType(nskill, sformindex, dformindex);
                        }
                        else
                        {
                            if (baddamage == 0x1000)
                    ...
                    AfterTargetHPGet:
                        mp = nbCalc.nbGetKoukaMp(1, nskill, sformindex, dformindex, powerrate, cnt);
                        mp2 = nbCalc.nbGetKoukaMp(0, nskill, sformindex, dformindex, powerrate, cnt);
                        baddamage = 0;
                        uint hiteff = nbCalc.nbCheckHitEffect(nskill, sformindex, dformindex);
                        int hpchk = (hp & hp2) >> 0x1f | canttarget;
                        int id = 0;
                        if (hpchk != 0)
                        {
                            if (datNormalSkill.tbl.Length <= nskill)
                                { return false; }
                            if (datNormalSkill.tbl[nskill].targettype == 0)
                                { skilltargetchk = true; }
                            if (hp > 0)
                            {
                                if (a.timelist[dformindex].hp <= hp)
                                {
                                    int clampchk = nbCalc.nbCheckKuisibariSkill(dformindex);
                                    if (clampchk != 0)
                                    {
                                        hp2 = a.timelist[dformindex].hp;
                                        uniqueid = a.uniqueid;
                                        id = 0x159;
                                        hp = hp2 - 1;
                                        nbMakePacket.nbAddSkillNamePacket(frame, uniqueid, id);
                                        if (hp > 0)
                                        {
                                            int hp3 = hp;
                                            if (a.timelist[dformindex].hp < hp)
                                                { hp3 = a.timelist[dformindex].hp; }
                                            float drainrate = nbCalc.nbGetHpDrainRitu(nskill, sformindex, dformindex);
                                            if (drainrate > 0.0f)
                                                { drainedhp += (int)(float)(hp * drainrate); }
                                            if ((((a.data.stat + 2) >> 1) & 1) == 0)
                                                { a.timelist[dformindex].hp -= (ushort)hp; }
                                            if (a.timelist[dformindex].hp == 0)
                                            {
                                                if((nbCalc.nbGetDevilFormatFlag(dformindex) >> 8 & 1) == 0)
                                                {
                                                    chkifdead = nbCalc.nbGetDeadType(nskill, sformindex, dformindex);
                                                    badstat = nbCalc.nbGetBadStat(dformindex);
                                                    if (badstat == 0x400)
                                                        { chkifdead = 1; }
                                                }
                                            }
                                            ...
                                            {
                                                chkifdead = nbCalc.nbCheckStoneDead(nskill, dformindex);
                                                if (chkifdead != 0)
                                                    { chkifdead = 1; }
                                            }
                                        }
                            ...
                            if (mp > 0)
                            {
                                float drainrate = nbCalc.nbGetMpDrainRitu(nskill, sformindex, dformindex);
                                if (drainrate > 0.0f)
                                    { drainedmp += (int)((float)mp * drainrate); }
                            }
                            uint flag = nbCalc.nbGetDevilFormatFlag(dformindex);
                            nbMakePacket.nbMakeDamagePacket(frame, uniqueid, ref form, hittype, hp, mp, (int)badstat, (int)hiteff, chkifdead, nbCalc.nbGetDeadType(nskill, sformindex, dformindex), 0, 0);
                            uint se = 0x10002;
                            if (koukatype - 1 > 1 && (nskill - 0xa4 < 3 || nskill == 0))
                            {
                                if (hittype == 1)
                                    { se++; }
                                nbMakePacket.nbAddSePacket(frame, uniqueid, se);
                            }
                            nbActionProcess.SetTimeList(ref a, dformindex, frame, 0x1e, 0x1e);
                            frame += 0x1e;
                        }
                        float gensyorate = nbCalc.nbGetGensyoRitu(nskill, sformindex, dformindex);
                        if (canttarget != 0)
                            { goto AfterHitCountLoop; }
                        if (hp < 1 && mp < 1 && hpchk == 0)
                        {
                            if ((0 | badrecover | datNormalSkill.tbl[nskill].badtype) == 0)
                            {
                                nbMakePacket.nbAddMojiEffKoukaPacket(frame, frame, uniqueid, ref form, 3);
                                if(a.hitstopframe == 0 || frame + 7 < a.hitstopframe)
                                {
                                    hittype = 0;
                                    a.hitstopframe = frame + 7;
                                }
                            }
                        }
                        powerrate *= gensyorate;
                        i++;
                    }
                    while (i < cnt);
                AfterHitCountLoop:
                    a.allhpdamage = drainedhp;
                    a.allmpdamage = drainedmp;
                    if (badrecover != 0 || (mp > 0) || (hp > 0))
                    {
                        if (mp < 1)
                            { mp = -1; }
                        if (hp < 1)
                            { hp = -1; }
                        nbMakePacket.nbMakeKaifukuPacket(frame, uniqueid, ref form, hittype, hp, mp, badstat);
                        if (badrecover != 0)
                            { nbCalc.nbSetBadStat(dformindex, 0, (int)badrecover); }
                        if (hp > 0)
                        {
                            hp += a.timelist[dformindex].hp;
                            if (hp > -1)
                                { a.timelist[dformindex].hp = (ushort)hp; }
                            else
                                { a.timelist[dformindex].hp = 0; }
                        }
                        nbActionProcess.SetTimeList(ref a, dformindex, frame, 0xf, 0x14);
                        frame += 0xf;
                    }
                    if (dformindex < 0x10)
                    {
                        if (datNormalSkill.tbl[nskill].deadtype != 0)
                        {
                            form = a.data.form[dformindex];
                            nbMakePacket.nbAddHojoKoukaPacket(frame, uniqueid, ref form, datNormalSkill.tbl[nskill].hojotype, datNormalSkill.tbl[nskill].hojopoint);
                            nbActionProcess.SetTimeList(ref a, dformindex, 10, 0xf, endframe);
                            form = a.form;
                        }
                    }
                    if (hp > 0 || mp > 0)
                    {
                        if (mp < 1)
                            { mp = -1; }
                        form = a.data.form[sformindex];
                        nbMakePacket.nbMakeKaifukuPacket(frame + 0xf, uniqueid, ref form, 2, hp, mp, badstat);
                        form = a.form;
                        if (hp < 0)
                        {
                            if((a.stat >> 1 & 1) == 0)
                            {
                                hp += a.timelist[sformindex].hp;
                                if (hp < 0)
                                    { a.timelist[sformindex].hp = 0; }
                                else
                                    { a.timelist[sformindex].hp = (ushort)hp; }    
                            }
                        }
                        nbActionProcess.SetTimeList(ref a, sformindex, frame + 5, 0xf, 0x14);
                    }
                    if (skilltargetchk || a.type != 0)
                    {
                        nbCalc.nbSetKuisibariStop(dformindex);
                        nbActionProcess.SetDeadPacket(ref a, dformindex);
                    }
                    else
                    {
                        nbActionProcess.CheckCounter(ref a, dformindex, sformindex);
                        nbCalc.nbSetKuisibariStop(dformindex);
                    }
                    if (nskill - 0x31 < 2)
                        { nbMisc.nbSetJyokyoMes(ref a, 0x4e, 1, d.id); }
                    else if(nskill == 0xe0)
                        { nbMisc.nbSetJyokyoMes(ref a, 0x58, 1, d.id); }
                    if (hittype == 2)
                    {
                        if (-1.0f < a.press && (a.press < -0.5f || a.press == 0.0f))
                            { a.press -= 0.5f; }
                        if (1 < a.newpresstype)
                            { return false; }
                        a.newpresstype = 2;
